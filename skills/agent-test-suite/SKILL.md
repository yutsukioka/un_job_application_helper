...
1. Verifies that `inputs/application_context.md` (or a designated test
...
- `inputs/application_context.md` (or a test-specific context file) with
...
1. **Read-only on production outputs.** Do not overwrite files in
   `output/generated_documents/`. Write all diagnostic artifacts to
...
3. **Use actual skills.** Do not reference skills that do not exist in
   `.agents/skills/`. The test validates the real skill registry.
...
1. Read `inputs/application_context.md`.
...
1. List all directories under `.agents/skills/` that contain a
   `SKILL.md`.
...
2. Verify each claim appears in `inputs/application_context.md`.
...
Write `output/tmp/test_suite/0x_agent_diagnostic_report.md` with these
sections:
...
File: `output/tmp/test_suite/0x_agent_diagnostic_report.md`
