...
This skill reads one or more SKILL.md files from `.agents/skills/` and
...
- `batch_mode: true`: analyze all skills under `.agents/skills/`.
...
| Check | Severity | Pass Condition |
|---|---|---|
| Output artifact specified | ERROR | File path stated (e.g., `output/tmp/...`) |
...
1. Identify target skill(s): read the specified SKILL.md file or scan
   all `.agents/skills/*/SKILL.md` in batch mode.
2. For each SKILL.md:
...
File: `output/tmp/0x_prompt_repair_log.md`
