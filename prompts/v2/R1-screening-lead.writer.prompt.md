...
4. Join the coordination server with:
   python .agents/agent_sync/client_v6.py join screening-lead --port 9860
...
1. IMPLEMENT pass 1:
   ...
   - Call:
     python .agents/agent_sync/client_v6.py impl-done screening-lead --summary "<short>" --port 9860
2. TEST / DISCUSS / loop:
   - Standard author-server pattern. Do not call `test-result` or
     `discuss-done --next-impl`.
   - During DISCUSS call:
     python .agents/agent_sync/client_v6.py discuss screening-lead "<structured discuss>" --port 9860
     python .agents/agent_sync/client_v6.py discuss-done screening-lead --port 9860
