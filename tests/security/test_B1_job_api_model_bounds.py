...
def test_B1_search_request_rejects_unbounded_payloads(tmp_path: Path, payload: dict[str, object]) -> None:
    response = _client(tmp_path).post("/api/search", json=payload)

    assert response.status_code == 422


def test_B1_tracker_record_rejects_oversized_notes(tmp_path: Path) -> None:
