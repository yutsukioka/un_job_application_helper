...
The saved-search command uses a full decoded path capture internally so historical names containing `/`, as well as the empty name accepted by the existing store, remain addressable after URL encoding. The tracker command likewise captures complete decoded nonempty IDs containing `/`. Neither command normalizes or reinterprets its identifier: the captured path value must still equal the exact expected snapshot identifier.
