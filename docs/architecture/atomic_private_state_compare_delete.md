...
The canonical path identifier is an opaque fixed-length token: `~sha256-` followed by lowercase SHA-256 of the exact identifier's UTF-16BE code units without a byte-order mark. Unpaired surrogate code units are preserved. This representation is reproducible from Dart `String.codeUnits` and keeps slash, control-character, dot-segment, empty, long, and surrogate-bearing saved-search names addressable without placing the raw value in the URL. It does the same for nonempty tracker IDs. The server recomputes the token from the strict expected snapshot and requires an exact match before store access. Raw single-segment identifiers remain accepted for compatibility, but reviewed migration clients use the opaque token.

The path identifier must equal either the expected snapshot identifier or its canonical opaque token. An identity mismatch returns fixed HTTP 400. Exact current content is deleted and returns `{"outcome":"deleted"}`. A missing current record is idempotent and returns `{"outcome":"absent"}`. Changed current content returns fixed HTTP 412 and remains untouched.
