...
```
private/output/generated_documents/history/<position-name>/
...
Operational notes:
- Launch each server from its own run directory (e.g., `private/tmp/agent_sync/p0a`,
  `private/tmp/agent_sync/p0b`, `private/tmp/agent_sync/s1`, …) so v6 logs do not collide.
...
`private/tmp/_budget_<server>.json`; each advisor increments its own message
...
|-------|--------|----------|
| JD text | `## JOB_DESCRIPTION_TEXT` section of `private/inputs/application_context.md` ONLY | YES |
...
- `## APPLICANT_PROFILE` or any other section of `private/inputs/application_context.md`
- `metric_ledger.md`
- `phase1_2_core_requirements.md`
- `phase1_7_strategy_report.md`
- `ccog_reference_resolved.md`
...
- For each factual claim in the Option documents, verify that it has
  ...
  - `private/inputs/application_context.md` (for biographical/qualification claims)
