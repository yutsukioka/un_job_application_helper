...
def sync_source(
    ...
    *,
    ...
    adapter = adapter_cls(AdapterContext(source=source, http=http, robots=RobotsChecker(policy)))
    try:
    ...
    if len(jobs) > policy.max_jobs_per_source:
        ...
        jobs = jobs[: policy.max_jobs_per_source]
    ...
    if close_missing:
        missing_counts = db.mark_missing(
            source.id,
            {job.identity_key() for job in jobs},
...
def sync_source_with_selective_details(
    ...
    *,
    ...
    if len(listing_jobs) > policy.max_jobs_per_source:
        result.errors.append(
            f"Adapter returned {len(listing_jobs)} jobs, above max_jobs_per_source={policy.max_jobs_per_source}"
        )
        listing_jobs = listing_jobs[: policy.max_jobs_per_source]
