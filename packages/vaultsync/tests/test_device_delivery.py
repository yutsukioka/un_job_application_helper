...
def vector():
    return json.loads(
        (ROOT / "contracts/sync/test_vectors/atlasvault_device_delivery_v2.json").read_text()
    )
...
def test_v2_shared_signature_preserves_wrapper_and_v1_bytes():
    ...
    created = create_device_delivery(
        source,
        recipient_device_id=expected["recipient_device_id"],
        issuer_device_id=p["rotation_signer_device_id"],
        signing_key=Ed25519PrivateKey.from_private_bytes(bytes([10]) * 32),
        current_registry=verify_transition(p["revocation"], p["registry"]),
        recovery_pending=False,
    )
    assert created == expected["packet"]
    assert created["proof"]["rotation_signer_device_id"] == p["rotation_signer_device_id"]
    assert created["wrapper"] == next(
        d for d in p["deliveries"] if d["device_id"] == expected["recipient_device_id"]
    )
    ...
    verify_epoch_rotation(
        p,
        registry=p["registry"],
        account_id=p["plan"]["account_id"],
        vault_id=p["plan"]["vault_id"],
        previous_epoch=3,
...
def test_foreign_wrapper_wrong_context_and_v1_only_are_rejected():
    ...
    packet["wrapper"] = next(
        d
        for d in original()["proof"]["deliveries"]
        if d["device_id"] != vector()["recipient_device_id"]
    )
    with pytest.raises(RotationError):
        verify(packet)
    with pytest.raises(RotationError, match="ATLAS_PER_DEVICE_PROOF_REQUIRED"):
        verify(original())
    for context in (
        {"account_id": "other"},
        {"vault_id": "other"},
...
def test_revoked_or_recovery_pending_issuer_cannot_upgrade():
    ...
    with pytest.raises(RotationError):
        create_device_delivery(source, **args, recovery_pending=True)
    after = copy.deepcopy(after)
    for e in after:
        if e["device_id"] == args["issuer_device_id"]:
            e["state"] = "REVOKED"
    args["current_registry"] = after
    with pytest.raises(RotationError):
        create_device_delivery(source, **args, recovery_pending=False)
...
def test_v2_schema():
    ...
    jsonschema.validate(
        vector()["packet"],
        json.loads((ROOT / "contracts/sync/atlasvault_device_delivery_v2.schema.json").read_text()),
    )
