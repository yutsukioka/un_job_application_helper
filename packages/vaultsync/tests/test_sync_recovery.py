...
V = json.loads(
    (ROOT / "contracts/sync/test_vectors/atlasvault_sync_recovery_vectors_v1.json").read_text()
)
P = V["packets"]
...
def client(path):
    return GuardedSyncState(
        ...
        key_epoch=2,
        trusted_signer=base64.b64decode(V["signing_public_b64"]),
    )
...
def ready(path):
    ...
    c.initialize()
    for n in ["one", "two"]:
        assert MaliciousServer(P[n]).serve(c)
    return c
...
def test_hostile_delivery_fences_without_cursor_or_terminal_change(tmp_path, attack):
    ...
    b = ready(tmp_path / "B")
    before = b.checkpoint()
    assert before == V["expected_checkpoint"]
    with pytest.raises(StateViewError, match=attack["reason"]):
        MaliciousServer(P[attack["packet"]]).serve(b)
    b = client(tmp_path / "B")
    ...
    with pytest.raises(StateViewError, match="ATLAS_RECOVERY_PENDING"):
        b.automatic_sync(lambda: calls.append(1))
    with pytest.raises(StateViewError, match="ATLAS_RECOVERY_PENDING"):
        MaliciousServer(P["three"]).serve(b)
    assert not calls
...
def test_manual_fork_preserves_both_branches_and_never_rewrites(tmp_path):
    ...
    b = client(tmp_path / "B")
    b.initialize()
    for n in ["one", "fork_two"]:
        MaliciousServer(P[n]).serve(b)
    left = a.export_evidence()
    right = b.export_evidence()
    before = a.checkpoint()
    with pytest.raises(StateViewError, match="ATLAS_STATE_EQUIVOCATION"):
        a.compare_evidence(right)
    evidence = a.evidence()
    assert evidence["local"] == left and evidence["peer"] == right
    ui = a.recovery()
    assert (
        a.resolve("select_peer", ui["local"][-1]["root"], ui["peer"][-1]["root"])
        == "RECOVERY_PENDING"
    )
    a = client(tmp_path / "A")
...
def test_explicit_reject_known_replay_resumes_without_forgetting_evidence(tmp_path):
    ...
    ui = c.recovery()
    with pytest.raises(StateViewError):
        c.resolve("retain_accepted", "f" * 64, ui["peer"][-1]["root"])
    ...
    before = c.checkpoint()
    assert not MaliciousServer(P["three"]).serve(c)
...
def crash_after_alarm(path, signal):
    c = ready(Path(path))
    try:
        MaliciousServer(P["fork_two"]).serve(c)
...
def test_alarm_survives_process_kill(tmp_path):
    ...
    child = multiprocessing.get_context("spawn").Process(
        target=crash_after_alarm, args=(str(path), str(signal))
    )
    child.start()
    try:
        ...
        with pytest.raises(StateViewError):
            client(path).automatic_sync(lambda: None)
    finally:
        if child.is_alive():
            child.kill()
            child.join(10)
