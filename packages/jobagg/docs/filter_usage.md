...
```bash
jobagg audit-classification --all --format markdown --output private/jobagg/output/classification_audit.md
```
...
```bash
jobagg saved-search run nairobi-international-p2-p4 --format markdown --output private/jobagg/output/nairobi_p2_p4.md
```

Run all saved searches:

```bash
jobagg saved-search run --all --format json --output private/jobagg/output/saved_search_runs.json
```
