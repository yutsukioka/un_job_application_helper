...
The aggregator has a complete current listing snapshot for the successfully fetched enabled sources in the latest full live validation run. That snapshot is stored under `private/jobagg/output/_rigorous_20260520` and contains:
...
The regular root database `private/jobagg/output/all_jobs.sqlite3` may be older than the latest rigorous validation artifact. Use the newest validated output directory when checking current coverage.
...
```bash
.venv/bin/jobagg sync-bundles --output-dir private/jobagg/output
```
...
11. Refresh the consolidated `private/jobagg/output/all_jobs.sqlite3` database by default after bundle publication. Use `--skip-consolidate` only when you intentionally want per-source bundle output without refreshing the single all-jobs database.
...
.venv/bin/jobagg sync-bundles \
  ...
  --output-dir private/jobagg/output/undp_refresh \
  --refresh-all-details
```
...
```bash
.venv/bin/jobagg sync-bundles --source-id undp_oracle_hcm --output-dir private/jobagg/output --refresh-all-details
```
...
.venv/bin/jobagg consolidate-bundles \
  ...
  --summary-output private/jobagg/output/organization_summary.csv
```
...
```bash
.venv/bin/jobagg ops-check --all --output private/jobagg/output/ops_check.md
```

Classification audit:

```bash
.venv/bin/jobagg --db private/jobagg/output/all_jobs.sqlite3 audit-classification \
  --all \
  --format markdown \
  --output private/jobagg/output/classification_audit.md
```

Search with explanations:

```bash
.venv/bin/jobagg --db private/jobagg/output/all_jobs.sqlite3 search \
  --city Nairobi \
  --country KE \
  --scope international \
  --grade P2 \
