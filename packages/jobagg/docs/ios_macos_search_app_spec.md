...
- Per-organization bundles plus consolidated `private/jobagg/output/all_jobs.sqlite3`, `all_jobs_current.json`, and `all_jobs_history.json`.
...
- Own `private/jobagg/output/all_jobs.sqlite3` and per-source bundles.
...
{
  "status": "ok",
  "db_path": "private/jobagg/output/all_jobs.sqlite3",
...
1. Server runs `jobagg sync-bundles --output-dir private/jobagg/output`.
...
- A Mac client can search `private/jobagg/output/all_jobs.sqlite3` through the local API without loading all jobs into client storage.
