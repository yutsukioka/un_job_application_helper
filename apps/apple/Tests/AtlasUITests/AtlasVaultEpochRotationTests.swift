...
final class AtlasVaultEpochRotationTests: XCTestCase {
  func vector() throws -> [String: Any] {
    ...
    return try JSONSerialization.jsonObject(with: Data(contentsOf: root.appendingPathComponent("contracts/sync/test_vectors/atlasvault_epoch_rotation_v1.json"))) as! [String: Any]
  }
  func check(_ proof: [String: Any], original: [String: Any]) throws -> [String: Any] {
    try AtlasVaultEpochRotation.verify(proof, registry: original["registry"] as! [[String: Any]],
  ...
  func testSharedSignedBindingAndActiveRecipients() throws {
    ...
    XCTAssertFalse((result["recipients"] as! [String]).contains((vectors["device_ids"] as! [String])[2]))
  }
  func testEachEpochContextFieldRejectsSubstitution() throws {
    ...
    for field in (proof["plan"] as! [String: Any]).keys {
      var changed = proof, plan = proof["plan"] as! [String: Any]
      if let value = plan[field] as? Int { plan[field] = value + 1 }
      else if plan[field] is [String] { plan[field] = [String]() }
      else { plan[field] = "substituted" }
      changed["plan"] = plan
