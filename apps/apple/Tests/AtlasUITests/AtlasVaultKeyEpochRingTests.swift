...
final class AtlasVaultKeyEpochRingTests: XCTestCase {
    func testEpochRingMatchesSharedMetadataAndRecordDerivation() throws {
        ...
        XCTAssertTrue(NSDictionary(dictionary: ring.metadata.jsonObject).isEqual(to: try object(try object(root, "ring"), "metadata")))
        XCTAssertEqual(ring.currentKeyEpoch, 3)
        XCTAssertEqual(
            sha256Hex(try ring.deriveRecordKey(
                keyEpoch: 1,
    ...
    func testEpochRingMigratesLegacyAndRecoversRetainedKey() throws {
        ...
        XCTAssertTrue(NSDictionary(dictionary: migrated.metadata.jsonObject).isEqual(to: try object(legacy, "expected_metadata")))
    ...
    func testEpochRingRejectsAmbiguousOrUnboundedState() throws {
        let root = try loadRoot()
        let keys = Dictionary(uniqueKeysWithValues: (1...33).map {
    ...
    private func makeRing(_ root: [String: Any]) throws -> AtlasVaultKeyEpochRing {
        ...
        return try AtlasVaultKeyEpochRing(
            ...
            keys: Dictionary(uniqueKeysWithValues: try entries.map { entry in
                (
    ...
    private func loadRoot() throws -> [String: Any] {
        ...
        let candidates = [
            URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
                .appendingPathComponent("../../contracts/sync/test_vectors/atlasvault_key_epoch_vectors_v1.json"),
            source.appendingPathComponent("../../../../contracts/sync/test_vectors/atlasvault_key_epoch_vectors_v1.json"),
        ]
