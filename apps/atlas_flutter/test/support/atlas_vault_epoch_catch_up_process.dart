...
Future<void> main(List<String> args) async {
  ...
  final c = AtlasVaultEpochVault(
    root,
    storageKey: Uint8List.fromList(List.filled(32, 52)),
    deviceID: (v['device_ids'] as List)[2] as String,
  ...
  if (!await File('${root.path}/activation').exists()) {
    ...
    final h = AtlasVaultGuardedSyncState(
      file: File('${root.path}/initial-history'),
      encryptionKey: Uint8List.fromList(List.filled(32, 62)),
      accountId: view['account_id'] as String,
      vaultId: 'vault-c26',
  ...
  Future<void> point(String name) async {
    if (name == stage) {
      await File('${root.path}/ready').writeAsString(name, flush: true);
      while (true) {
  ...
  if (stage == 'observe') {
    try {
      ...
      await c.recoverPublication();
    }
    final o = await c.observation();
    var fenced = false;
    try {
      await c.seal(
        'patch',
        ...
        objectID: 'probe',
        revision: 'r1',
        signingKey: await Ed25519().newKeyPairFromSeed(List.filled(32, 12)),
      );
    } catch (_) {
      fenced = true;
    }
    stdout.writeln(jsonEncode({...o, 'writes_fenced': fenced}));
    return;
  }
  if ((await c.observation())['status'] != 'CLEANUP_PENDING') {
    await c.catchUpForTesting(
      packets,
      currentActivationID: v['target_activation_id'] as String,
      agreementPrivateKey: Uint8List.fromList(List.filled(32, 22)),
      historyUpdates: rows(v['history_updates']),
      checkpoint: point,
    );
  }
  if (stage == 'cleanup_pending' ||
      stage == 'deleted_epoch' ||
      stage == 'cleanup_resume' ||
      (await c.observation())['status'] == 'CLEANUP_PENDING') {
    await c.cleanupEpochsForTesting(
      retainEpochs: {5},
      deleteEpoch: (e) async {
        await File(
          '${root.path}/deleted-$e',
        ).writeAsString('deleted', flush: true);
      },
      containsEpoch: (e) async =>
          !await File('${root.path}/deleted-$e').exists(),
      checkpoint: point,
    );
  }
