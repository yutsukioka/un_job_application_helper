...
void main() {
    ...
    test('fresh platform removal prompt is strict on $platform', () async {
      ...
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
          .setMockMethodCallHandler(channel, (call) async {
            expect(call.method, 'authorizeDeviceRemoval');
            expect(call.arguments, isNull);
            calls++;
            if (result is Exception) throw result;
            return result;
          });
      final auth = AtlasVaultDeviceRemovalAuthorizer(
        platform: platform,
        channel: channel,
      );
      for (result in [
        ...
        1,
        {'ok': true},
        PlatformException(code: 'private-sentinel'),
      ]) {
        expect(await auth.authorize(), isFalse);
      }
      result = true;
      expect(await auth.authorize(), isTrue);
      expect(await auth.authorize(), isTrue);
      expect(calls, 8);
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
  ...
  testWidgets('exact target confirmation, safe UI and disposal cancellation', (
    tester,
  ) async {
      ...
      try {
        final v =
            jsonDecode(
                ...
                as Map;
        final entries = (v['registry'] as List)
        ...
        await tester.pumpWidget(
          MaterialApp(
            home: AtlasVaultDeviceRemovalView(
              controller: controller,
              targetDevice: entries[1]['device_id'] as String,
              keyEpoch: 3,
            ),
          ),
        );
        ...
        for (
          var i = 0;
          i < 100 && find.text('Registry: ACTIVE').evaluate().isEmpty;
          i++
        ) {
          await Future<void>.delayed(const Duration(milliseconds: 10));
          await tester.pump();
        }
        expect(find.text('Registry: ACTIVE'), findsOneWidget);
        expect(prompts, 0);
        final allText = [
          ...tester
        ...
        expect(allText, contains('3'));
        expect(allText, contains('4'));
        expect(
