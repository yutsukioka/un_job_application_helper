...
1. `private/inputs/application_context.md` exists with at minimum:
   - `## RUN_MODE` (may be empty lists for single-agent mode)
   - `## BUDGETS`
   - `## LIMITS` (TARGET_SYSTEM)
2. Vacancy-specific output directory exists:
   `private/output/generated_documents/history/<position>/`
3. `private/tmp/agent_sync/` is writable.
...
- Initialize `<OUTDIR>/_discussion/run_manifest.json` with
  `agents/apex/scripts/write_run_manifest.py init`.
...
- [ ] `_discussion/advisor_notes_<server>.md` contains at least one
  ...
  Validate with:
  `python agents/apex/scripts/validate_advisor_notes.py --advisor-notes <OUTDIR>/_discussion/advisor_notes_<server>.md --advisors "<advisor1>,<advisor2>" --json`
...
`python agents/apex/scripts/prepare_d_writer_scaffold.py --outdir <OUTDIR> --role <agent>`
...
They must not read full `private/inputs/application_context.md`, `metric_ledger.md`,
...
- Archive `private/tmp/agent_sync/<server>/` directories to a run-specific
