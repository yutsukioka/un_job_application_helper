...
Purpose:
- read `private/inputs/application_context.md`
...
`private/inputs/application_context.md` has a non-empty `ENSEMBLE_PHASE_1_7` or
...
- If `MODE: ensemble_v2` (or either ensemble list is non-empty), use the
  per-server prompts under `agents/apex/prompts/v2/` and the launcher at
  `agents/apex/scripts/launch_v2_servers.sh`.
- Per-server registry, ports, writer/advisors and canonical tester are
  defined in `agents/apex/topology/server_manifest.yaml`. The launcher
...
```bash
python agents/apex/scripts/validate_advisor_notes.py \
  --advisor-notes private/output/generated_documents/history/<JOB_SLUG>/_discussion/advisor_notes_<SERVER>.md \
...
They must not read:
- full `private/inputs/application_context.md`
- candidate history
- `metric_ledger.md`
- `phase1_7_strategy_report.md`
- D1/D2/D3 drafts
