...
4. Join the coordination server with:
   python agents/apex/agent_sync/client_v6.py join qa-auditor --port 9820
...
1. IMPLEMENT pass 1:
   ...
   - Record the consensus skill chain:
     python agents/apex/scripts/write_run_manifest.py add-skill --outdir <OUTDIR> --skill apex-orchestrator-report --server C1 --artifact phase1_7_strategy_report.md
...
4. Wait until all three advisors have called `discuss-done` without
   ...
   the loop), then end the stage with the separate `shutdown` command:
     python agents/apex/agent_sync/client_v6.py discuss-done qa-auditor --next-impl qa-auditor --port 9820   # loop
     python agents/apex/agent_sync/client_v6.py discuss-done qa-auditor --port 9820                          # close, no loop
     python agents/apex/agent_sync/client_v6.py shutdown --reason "C1 complete" --port 9820                  # end stage
