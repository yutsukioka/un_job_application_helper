...
4. Join the coordination server with:
   python agents/apex/agent_sync/client_v6.py join screening-lead --port 9811
...
3. DISCUSS: submit one structured `discuss`, then call `discuss-done`
   **without** `--next-impl` so the DISCUSS barrier can advance:
     python agents/apex/agent_sync/client_v6.py discuss   screening-lead "<text>" --port 9811
     python agents/apex/agent_sync/client_v6.py discuss-done screening-lead --port 9811
