...
4. Join the coordination server with:
   python agents/apex/agent_sync/client_v6.py join screening-lead --port 9831
...
- `apex-user-feedback-revision` (Phase 7.5) has run; any approved updates
  in `private/inputs/user_feedback_updates.md` are in scope per
...
1. IMPLEMENT pass 1:
   - Run the D-writer scaffold preflight before drafting:
     python agents/apex/scripts/prepare_d_writer_scaffold.py --outdir <OUTDIR> --role screening-lead
