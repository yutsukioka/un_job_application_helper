...
3. Load `agents/apex/.github/agents/ats-format-lead.agent.md` and apply its
   **Context Scoping**, **Voice & Emphasis**, and **Writer Mode (v2)**
   sections.
4. Join the coordination server with:
   python agents/apex/agent_sync/client_v6.py join ats-format-lead --port 9833
...
1. IMPLEMENT pass 1:
   - Run the D-writer scaffold preflight before drafting:
     python agents/apex/scripts/prepare_d_writer_scaffold.py --outdir <OUTDIR> --role ats-format-lead
